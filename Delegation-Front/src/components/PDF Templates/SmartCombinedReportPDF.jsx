    ...
    <Document key={'smart-combined-report'}>
            ...
            <View style={styles.table}>
              ...
              {delegationData.map((row, i) => (
                <View style={styles.row} key={i}>
                  ...
                  <Text style={styles.cell}>{row.arrivalInfo?.arrivalOrigin || '-'}</Text>
      ...
      {showMembers && (
        <Page size="A4" orientation="landscape" style={styles.page}>
          ...
          <Text style={styles.header}>بيانات أعضاء الوفود</Text>
                        ...
                        {delegationMembers.map((member, i) => {
                          ...
                          return (
                            <View style={styles.row} key={i}>
                              ...
                              <Text style={styles.cell}>{member.equivalent_job_name || member.job_title || '-'}</Text>
                              <Text style={styles.cell}>
                                {member.status === 'DEPARTED' ? 'غادر' :
                                 member.status === 'NOT_DEPARTED' ? 'لم يغادر' : '-'}
                              </Text>
                              {/* قادم من - من بيانات الوفد */}
                              <Text style={styles.cell}>{memberDelegation?.arrivalInfo?.arrivalOrigin || '-'}</Text>
                              {/* تاريخ الوصول - من بيانات الوفد */}
                              <Text style={styles.cell}>{memberDelegation?.arrivalInfo?.arrivalDate || '-'}</Text>
                              {/* سعت الوصول - من بيانات الوفد */}
                              <Text style={styles.cell}>{memberDelegation?.arrivalInfo?.arrivalTime || '-'}</Text>
                              {/* رقم رحلة الوصول - من بيانات الوفد */}
                              <Text style={styles.cell}>{memberDelegation?.arrivalInfo?.arrivalFlightNumber || '-'}</Text>
                              {/* شركة طيران الوصول - من بيانات الوفد */}
                              <Text style={styles.cell}>{memberDelegation?.arrivalInfo?.arrivalAirline || '-'}</Text>
